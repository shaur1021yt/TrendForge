...
import { getToolBySlug, trackPageView } from '@/lib/data';
import { Flame, Clock, Share2, Bookmark, AlertTriangle, ArrowRight } from 'lucide-react';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: tool.meta_title || tool.title, description: tool.meta_description || tool.description, type: 'website' },
  };
}
...
export default async function ToolPage({ params }: Props) {
  ...
  try { await trackPageView(tool.id); } catch {}
  ...
  const structuredData = JSON.parse(tool.structured_data || '{}');
  const faqs = structuredData.faqs || [];

  // Get related tools
  const relatedTools: typeof tool[] = [];
  for (const s of relatedSlugs.slice(0, 4)) {
    const t = await getToolBySlug(s);
    ...
    <div className="min-h-screen bg-background">
      <nav className="border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button className="p-2 hover:bg-muted rounded-lg transition-colors" title="Share"><Share2 className="h-5 w-5 text-muted-foreground" /></button>
            <button className="p-2 hover:bg-muted rounded-lg transition-colors" title="Save"><Bookmark className="h-5 w-5 text-muted-foreground" /></button>
          </div>
        </div>
      </nav>
      ...
      <article className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          ...
          <Link href="/tools" className="hover:text-foreground transition-colors">Tools</Link><span>/</span>
          <span className="text-foreground">{tool.title}</span>
        </div>

        <header className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-primary/10 text-primary capitalize">{tool.tool_type}</span>
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-muted text-muted-foreground capitalize">{tool.category}</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-4">{tool.title}</h1>
          <p className="text-lg text-muted-foreground">{tool.description}</p>
          <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground">
            <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> Last updated: {tool.date_published ? new Date(tool.date_published).toLocaleDateString() : 'Recently'}</span>
            <span>•</span><span>Version {tool.version}</span>
          </div>
        </header>

        <section className="mb-10">
          <ToolCalculator toolId={tool.id} slug={tool.slug} inputs={inputs} outputs={outputs} toolType={tool.tool_type} title={tool.title} />
        </section>

        {tool.content && <section className="prose prose-lg max-w-none mb-10" dangerouslySetInnerHTML={{ __html: tool.content }} />}
        ...
        {['finance', 'fitness'].includes(tool.category) && (
          <div className="flex items-start gap-3 p-4 bg-yellow-50 border border-yellow-200 rounded-xl mb-10 text-sm">
            ...
            <div>
              ...
              <p className="text-yellow-700 mt-1">This tool provides estimates for informational purposes only. It is not financial, medical, or professional advice.</p>
            </div>
          </div>
        )}
              ...
              {affiliateProducts.map((product, i) => (
                <div key={i} className="tool-card card-hover">
                  <div className="flex items-start justify-between">
                    ...
                    <a href="#" className="shrink-0 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">Check Price</a>
                  </div>
                </div>
              ))}
            </div>
            <p className="text-xs text-muted-foreground mt-3">* Product links may be affiliate links. We may earn a commission at no extra cost to you.</p>
          </section>
        )}
        ...
        {relatedTools.length > 0 && (
          <section className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {relatedTools.map(rt => (
                <Link key={rt.id} href={`/tools/${rt.slug}`} className="tool-card card-hover">
      ...
      <footer className="border-t border-border py-6 mt-10">
        <div className="max-w-4xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-2"><Flame className="h-4 w-4 text-orange-500" /><span className="font-medium">TrendForge AI</span></div>
